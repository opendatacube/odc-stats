...
FROM mambaorg/micromamba:git-37bfa82-jammy AS stats-conda

USER root
COPY --link docker/env.yaml /conf/
RUN micromamba env create -y -p /env -f /conf/env.yaml && \
...
COPY --link pyproject.toml setup.py setup.cfg README.md /build/
